...
> ⚠️ **#D 在 Phase 1（早于 Phase 2.0/2.1 用例解析）**：`{N}`（主集用例条数）需就地扫 `docs/testing/{version}/正式用例/` 用例文件**点算条目**（表格行 / `TC-*` 用例编号 / 用例标题；⛔ 别按 `T-NNN` 点算——那是 `dev-manual-testcase` 给**待澄清项**的前缀，算进来会让用例数虚高），`{K}` 扫 `研发自测/`；不得用「份数」顶替「例数」。详见 0.1bis 对称计数铁律。
