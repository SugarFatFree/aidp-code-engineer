...
Step 4.6: ★ 同族增量项静态门（约定 20 姊妹条「同族增量项」；纯静态、**前后端通吃**，脚本全栈扫描，
        放在前端段只因这类缺陷最常见于视觉家族）：
        执行：python3 {{AIDP_HOME}}/scripts/check_sibling_family.py --changed-only --json
        —— **本轮若往一个已有 ≥2 个同构成员的家族里加了新成员（运维入口 / 菜单项 / 版本区块 /
        增量脚本 / 索引条目 / 枚举项 / 页签…），必须看本门结论**。F0 声明漂移 / F1 成员自带外壳 /
        F2 成员未登记 = **Critical**；F3 顺序未数据化 / F4 疑似家族无公共外壳 = Important。
        —— 判定口径单一信源 = 该脚本 + rules/code.md 约定 20 姊妹条，命令端只编排不复述（约定 21）。
        —— 退出码：`0`=无 Critical · `1`=有 Critical · `2`=入参错（修参数重跑、不算违规）。
        Critical 当场修（抽公共外壳 / 补登记）；Important 逐条判断是真问题还是加
        `sibling-family-ignore: <F号> <原因>`（**原因必填**，空原因不放行）。
        —— **判据是「有没有兄弟」，不是「我写了几遍」**：家族已有成员 ≥2 即适用，哪怕本次只加 1 个。
        照着隔壁成员复制一份是执行体的默认动作，而失败形态是"看起来正常"——
        类型检查 / lint / 构建全绿，只有并排比对才看得见。
```
