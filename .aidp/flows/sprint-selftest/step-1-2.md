...
# 每段 bash 独立执行；仅 Git 索引内的源走 git mv，新产物/无 Git 走普通移动。
selftest_move() {
  if [ -e "$2" ] || [ -L "$2" ]; then
    printf '❌ 目标已存在，拒绝覆盖：%s\n' "$2" >&2
    return 1
...
#    输出在 {测试文档目录}/{项目名}/ 子目录、文件名已带 NN_ 前缀；命令端事后整体归一到 $DST_DIR/，
...
if [ -n "$PLAN_SRC" ]; then
  ...
  if [ "$SRC_DIR" != "$DST_DIR" ]; then
    ...
    for f in "$SRC_DIR"/*.md; do          # 整目录搬迁全部产物（含 00_索引.md / 01_研发自测方案.md / 02_~ 用例），NN_ 前缀原样保留
      [ -f "$f" ] || continue
      selftest_move "$f" "$DST_DIR/$(basename "$f")" || exit 1
    done
    [ "$SRC_DIR" != "$V_TEST" ] && [ -z "$(ls -A "$SRC_DIR" 2>/dev/null)" ] && rmdir "$SRC_DIR"
    echo "✅ 研发自测索引 + 方案 + 用例已整体归一到 $DST_DIR/（去 {项目名} 子目录、保留 NN_ 前缀）"
  fi
  # 归一到 SKILL 命名总则规范名：① 旧 SKILL 无前缀/带 {项目名}-v{版本} 后缀散文件 → 规范 NN_ 名（用例在 02_）；② 总览/单文件用例保留 SKILL 原名；③ stale 旧锚 00_方案归一到 01_（若已存在则冲突硬停）
  for f in "$DST_DIR"/*.md; do
    [ -f "$f" ] || continue
    case "$(basename "$f")" in
      全量自测用例-*)                       selftest_move "$f" "$DST_DIR/02_全量自测用例.md" || exit 1 ;;
      增量自测用例-*)                       selftest_move "$f" "$DST_DIR/02_增量自测用例.md" || exit 1 ;;
      02_自测用例-总览.md)                   : ;;   # 已是索引态规范名，保留
      02_*总览*.md)                         selftest_move "$f" "$DST_DIR/02_自测用例-总览.md" || exit 1 ;;
      00_研发自测方案.md)                    selftest_move "$f" "$DST_DIR/01_研发自测方案.md" || exit 1 ;;  # stale 旧锚归一到 01_；已有新方案则硬停
    esac
  done
...
调 SKILL 之前命令端必须执行：
  ...
  # 补充模式在 SKILL 调用前独立执行，不能依赖上面的 bash 会话函数。
  selftest_move() {
    if [ -e "$2" ] || [ -L "$2" ]; then
      printf '❌ 目标已存在，拒绝覆盖：%s\n' "$2" >&2
      return 1
  ...
  if [ -f "$V/研发自测用例.md" ]; then
    selftest_move "$V/研发自测用例.md" "$V/研发自测.md" || exit 1
  fi
  if [ -d "$V/研发自测用例" ]; then
    selftest_move "$V/研发自测用例" "$V/研发自测" || exit 1
  fi
  ...
  if [ -f "$MAIN_SINGLE" ] && [ ! -d "$MAIN_DIR" ]; then
    mkdir -p "$MAIN_DIR" || exit 1
    # ★ 用例主文档迁为 02_全量自测用例.md（索引态：00_ 索引 / 01_ 研发自测方案 / 用例从 02_ 起，约定 14/15 + 本命令 Step 1）
    selftest_move "$MAIN_SINGLE" "$MAIN_DIR/02_全量自测用例.md" || exit 1
  ...
  for f in "$V"/研发自测用例-补充-*.md "$V"/研发自测-补充-*.md; do
    [ -f "$f" ] || continue
    mkdir -p "$MAIN_DIR" || exit 1
    selftest_move "$f" "$MAIN_DIR/$(basename "$f")" || exit 1
  done
