...
`AIDP_HOME/commands/`、`AIDP_HOME/skills/`、`AIDP_HOME/plugins/` 始终是唯一真源。生成入口使用相对符号链接，
...
def _codex_command_preamble(name: str) -> str:
    return runtime_text(
        ...
        "`__AIDP_HOME__/commands/<命令名>.md`（此处命令名为 `foo`）。确认文件存在后，将 `args` 原样作为"
...
def _self_check() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        root = Path(td)
        (root / runtime_text('__AIDP_HOME__/skills/demo', __file__)).mkdir(parents=True)
        (root / runtime_text('__AIDP_HOME__/skills/demo/SKILL.md', __file__)).write_text(
            "---\nname: demo\ndescription: d\n---\n", encoding="utf-8")
        (root / runtime_text('__AIDP_HOME__/commands', __file__)).mkdir(parents=True)
        (root / runtime_text('__AIDP_HOME__/commands/sprint-dev.md', __file__)).write_text(
            "# /sprint-dev — 开发\n\n参数：$ARGUMENTS\n", encoding="utf-8")
        (root / runtime_text('__AIDP_HOME__/hooks', __file__)).mkdir(parents=True)
        (root / STOP_GUARD_REL).write_text("", encoding="utf-8")
        (root / "CLAUDE.md").write_text("# 正文\n", encoding="utf-8")
        (root / ".gitignore").write_text("node_modules/\n", encoding="utf-8")
        plugin = root / runtime_text('__AIDP_HOME__/plugins/demo-plugin', __file__)
        ...
        (root / runtime_text('__AIDP_HOME__/commands/sprint-dev.md', __file__)).unlink()
