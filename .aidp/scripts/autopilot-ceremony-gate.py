        ...
        if os.path.isfile(_tdjs):
            ...
            if not _actual:
                ...
                rec("报告driver如实性", bool(_waiver),
                    ...
                    f"（报告自述 driver={_claim or '(无)'}'）——驱动选定环节未走过。正确动作：跑 `python3 $AIDP_HOME/scripts/chrome-mcp-doctor.py check-cli` 选定驱动并写盘；若确有不可抗原因，须写 builds[].driver_actual_waiver 说明理由'")
