...
def run(root):
    ...
    if not rule_installed:
        findings.append({
            ...
            "detail": f"已启用 WebMCP，但详规未安装到 `{RULE_INSTALLED}'` —— rules 是**路径触发**加载的，文件不在那儿就永远不会自动加载，规则等于不存在。跑 `python3 $AIDP_HOME/scripts/check_webmcp.py --install-rule` 安装（模板位：'{RULE_TEMPLATE}）",
        })
