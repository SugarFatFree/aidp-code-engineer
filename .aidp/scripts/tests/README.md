...
| 42 | `check_version_identifier 多层反应堆 parent 同步` | — |
| 43 | `check_md_anchors 站内锚点死链` | 站内 `#锚点` 死链检出 |
| 44 | `check_ghost_flags 幽灵旗标 / 零误报` | 检出幽灵旗标 + 四类零误报（owner 判定 / 续行继承 / CSS 变量 / 假定义防护） |
| 45 | `check_loop_examples /loop 示例必带 --unattended` | 漏写检出 + 逐"出现"判 + 显式豁免 |
| 46 | `check_line_refs 硬编码行号引用` | 四种行号写法 + `L1-L3` 层级不误报 + 目标可解析性分组 |
| 47 | `check_shard_counts 分片自称片数 / 范围记法` | 片数少算检出 + 范围记法漏 `b` 分片检出 + 三类零误报（显式列出 / 序数「第 N 片」/ 跨命令同名前缀） |
| 48 | `check_sprint_numbering Sprint 编号跨版本 + 划分粒度` | 跨版本重号检出 + 同功能前后端被拆成两 Sprint 检出 |
| 49 | `tick flags：单 flag 解析 + 变量供给链` | 单 `--flag` 不再 argparse exit 2 + 登记变量必须有 set/回落/推导来源 |
| 50 | `骨架表 ↔ flow 分片 Step 覆盖` | 分片定义了 Step 但骨架表漏登记（= 定义了永不执行） |
| 51 | `跨文件长片段双写` | ≥150 字符逐字重复跨文件出现（改一处漏一处的漂移源） |
| 52 | `单一信源指针 + 分片标题风格` | 「单一信源 = X」指针指不到 + 同层级标题编号风格混用 |
| 53 | `WebMCP 启用判定 + 三项守卫` | 未启用必须彻底静默 + 按需安装闭环 + 判据归属 SKILL 的防回归断言 |
| 54 | `check_ui_fidelity R2/R3/R10 + 零误报` | 约定 39 三条确定性检查各自命中 + **每条配零误报反例**（颜色随值绑定 / 兄弟节点插值不算本标签内容 / 循环翻页捞全量 / 普通分页列表非导出）+ 豁免不串号 + R10 Critical 退出码 |
| 55 | `tick 供给链：动态兜底 / 关断旗标 / 回落键写入者` | 动态兜底档必须接进 `cmd_shell`；`--no-notify` 必须有生效路径；回落键须有真实写入者 |
| 56 | `check_upstream_call_log C1/C2/I1/I2/I3 + 零误报` | — |
| 57 | `check_skill_ref_freshness SKILL 编号引用新鲜度 + 零误报` | — |
| 58 | `tick 命名空间按命令隔离（两条 7×24 链路互不清空）` | — |
| 59 | `check_flow_bash_syntax 三类真错 + 零误报` | — |
| 60 | `check_chain_unattended G-CHAIN-1 棘轮` | — |
| 61 | `code_inventory 跨版本增量缓存` | — |
| 62 | `requirement_query 历史需求读时查询` | — |
| 63 | `archive_old_artifacts 老版本留仓归档` | — |
| 64 | `handback-check 中途交还控制权机器门` | — |
| 65 | `收尾门 3m 缺陷复验闭环` | — |
| 66 | `WebMCP 入口探测：白名单优先 + 全局扫描兜底` | — |
| 67 | `entry_mode build 级优先（版本级槽位互相覆盖的回归）` | — |
| 68 | `口述档位门有生产方 + 口径反转清算门` | — |
| 69 | `设计目标棘轮 + 实现名词体检` | — |
| 70 | `约定22 绕过反向判据 + 约定24 提交前门禁` | — |
| 71 | `文档编号连续性 + 检查脚本阳性对照骨架` | — |
| 72 | `yield 守卫棘轮 + HAS_WAKE_SOURCE 供给链` | — |
| 73 | `memory 整段被吞：确定性落点` | — |
| 74 | `约定 22 绕过检出：补「只改代码、册子零动」这一最常见形态` | — |
| 75 | `隐形漂移：内容变了但 git status 报 clean` | — |
| 76 | `失败处置五步：记账 → 判阈 → 冻结四件套 → 发 #4 → 让位` | — |
| 77 | `Mock 守卫策略按标记分流（两类目的相反，混用即出事）` | — |
| 78 | `收敛进脚本后，各门仍看得见` | — |
| 79 | `失败处置：--freeze-now / --extra / 枚举不可解析` | — |
| 80 | `CommonMark 围栏判定 + env/.env 键集新鲜度` | — |
| 81 | `收尾门 3p/3q 缺陷分流 + 上轮 block 复评` | — |
| 82 | `classify_push 同 build 多次推送累积` | — |
| 83 | `口径残留门 短形式词表 + 判决行` | — |
| 84 | `exec 报告继承 cases[]/testSummary` | — |
| 85 | `notify --section-file + 渲染冒烟检测面` | — |
| 86 | `设计目标三条形式落地的收口` | — |
| 87 | `--reset-* 旗标执行器` | — |
| 88 | `冻结字段写入契约` | — |
| 89 | `SKILL 阻断名单棘轮` | — |
| 90 | `术语一致性 + 约定 30 正文体检` | — |
| 91 | `台账多态识别 + 格式漂移告警` | — |
| 92 | `收尾门 3n 测试结果挂靠用例基线` | — |
| 93 | `子 Agent 派单契约（cascade_mode / dev_scale）` | — |
| 94 | `业务计数声明表全库回扫` | — |
| 95 | `约定 30 棘轮键：内容指纹而非行号` | — |
