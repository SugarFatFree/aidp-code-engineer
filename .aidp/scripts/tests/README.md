...
| 7 | `报告 na 结果态` | — |
| 8 | `约定 22 义务登记门` | — |
| 9 | `tick TARGET_VERSION 按命令分流` | — |
| 10 | `片数声明：路径归属 + 标识符不误取` | — |
| 11 | `SKILL 引用：assets 覆盖 + 安装态文件豁免` | — |
| 12 | `跨文件双写：整行片段` | — |
| 13 | `骨架表编号识别的两种形态` | — |
| 14 | `handback-check 自动触发点` | — |
| 15 | `tests/README 分组表行数` | — |
| 16 | `DEPLOY_MODE 的 PRD 兜底按版本取` | — |
| 17 | `研发执行计划 → Sprint 集合口径` | — |
| 18 | `实现偏离设计门` | — |
| 19 | `版本标识 · 继承 pom 透明化` | — |
| 20 | `台账终态门 --ledger-closed` | — |
| 21 | `全量设计 section 级前滚` | — |
| 22 | `SKILL 引用漂移门` | — |
| 23 | `autopilot 主干：转义 / 零写入方键 / 游标越权 / 冻结解冻` | — |
| 24 | `发布基线：布局探测 / 列注释误报 / 占位豁免 / build 补登记 / 版本改名` | — |
| 25 | `无人值守死锁 + 守卫旁路` | — |
| 26 | `SKILL doc_split 的 _ 前缀例外` | — |
| 27 | `链式调用免检变体 / SKILL 入参 / 尾段选版 / Stop hook / 分组计数` | — |
| 28 | `版本层字段 / 占位符 / 回落 / 枚举拆分` | — |
| 29 | `check_convention_dup 形态探测 + 双写检出` | **形态 A 分离型 / 形态 B 内联型都要真检** + 双写检出 + 跳过分类 |
| 30 | `check_version_identifier 版本标识对齐` | 漂移检出 / 归一化 / deny-list 后缀匹配 / 占位豁免 / pom 继承 / 依赖目录不下钻 |
| 31 | `baseline_edit run-state 推进判定 + current-version 选版` | run-state 状态机推进条件 + 当前版本选取 |
| 32 | `baseline_edit builds[] 寻址 + 切碎版本号护栏` | builds 数组按 build 号寻址 + 版本号被切碎的护栏 |
| 33 | `阶段完成判据 + 收尾义务清算` | autopilot 阶段完成判据 + 收尾义务是否被清算 |
| 34 | `通知门：build 口径计数 + 脚本自算期望集` | 里程碑通知按 build 计数、期望集由脚本自算而非人填 |
| 35 | `release_baseline_check 双轨部署基线 12 项机器门` | 约定 37 全量/增量基线的 12 项确定性校验 |
| 36 | `autopilot_stuck_check 既不失败也不推进的兜底熔断` | 7×24 空转（不报错也不前进）的熔断判据 |
| 37 | `ceremony-gate 台账：无 build 的卡可登记且被 check 认账` | 无 build 的里程碑通知也能登记并被闸门认账 |
| 38 | `notify 标题固定前缀：项目中文名称恒在` | — |
| 39 | `check_changelog_fix_scope 修复段准入判据` | — |
| 40 | `autopilot_tick_flags TARGET_VERSION 空目标可表达` | — |
| 41 | `check_version_identifier 多层反应堆 parent 同步` | — |
| 42 | `check_md_anchors 站内锚点死链` | 站内 `#锚点` 死链检出 |
| 43 | `check_ghost_flags 幽灵旗标 / 零误报` | 检出幽灵旗标 + 四类零误报（owner 判定 / 续行继承 / CSS 变量 / 假定义防护） |
| 44 | `check_loop_examples /loop 示例必带 --unattended` | 漏写检出 + 逐"出现"判 + 显式豁免 |
| 45 | `check_line_refs 硬编码行号引用` | 四种行号写法 + `L1-L3` 层级不误报 + 目标可解析性分组 |
| 46 | `check_shard_counts 分片自称片数 / 范围记法` | 片数少算检出 + 范围记法漏 `b` 分片检出 + 三类零误报（显式列出 / 序数「第 N 片」/ 跨命令同名前缀） |
| 47 | `check_sprint_numbering Sprint 编号跨版本 + 划分粒度` | 跨版本重号检出 + 同功能前后端被拆成两 Sprint 检出 |
| 48 | `tick flags：单 flag 解析 + 变量供给链` | 单 `--flag` 不再 argparse exit 2 + 登记变量必须有 set/回落/推导来源 |
| 49 | `骨架表 ↔ flow 分片 Step 覆盖` | 分片定义了 Step 但骨架表漏登记（= 定义了永不执行） |
| 50 | `跨文件长片段双写` | ≥150 字符逐字重复跨文件出现（改一处漏一处的漂移源） |
| 51 | `单一信源指针 + 分片标题风格` | 「单一信源 = X」指针指不到 + 同层级标题编号风格混用 |
| 52 | `WebMCP 启用判定 + 三项守卫` | 未启用必须彻底静默 + 按需安装闭环 + 判据归属 SKILL 的防回归断言 |
| 53 | `check_ui_fidelity R2/R3/R10 + 零误报` | 约定 39 三条确定性检查各自命中 + **每条配零误报反例**（颜色随值绑定 / 兄弟节点插值不算本标签内容 / 循环翻页捞全量 / 普通分页列表非导出）+ 豁免不串号 + R10 Critical 退出码 |
| 54 | `tick 供给链：动态兜底 / 关断旗标 / 回落键写入者` | 动态兜底档必须接进 `cmd_shell`；`--no-notify` 必须有生效路径；回落键须有真实写入者 |
| 55 | `check_upstream_call_log C1/C2/I1/I2/I3 + 零误报` | — |
| 56 | `check_skill_ref_freshness SKILL 编号引用新鲜度 + 零误报` | — |
| 57 | `tick 命名空间按命令隔离（两条 7×24 链路互不清空）` | — |
| 58 | `check_flow_bash_syntax 三类真错 + 零误报` | — |
| 59 | `check_chain_unattended G-CHAIN-1 棘轮` | — |
| 60 | `code_inventory 跨版本增量缓存` | — |
| 61 | `requirement_query 历史需求读时查询` | — |
| 62 | `archive_old_artifacts 老版本留仓归档` | — |
| 63 | `handback-check 中途交还控制权机器门` | — |
| 64 | `收尾门 3m 缺陷复验闭环` | — |
| 65 | `WebMCP 入口探测：白名单优先 + 全局扫描兜底` | — |
| 66 | `entry_mode build 级优先（版本级槽位互相覆盖的回归）` | — |
| 67 | `口述档位门有生产方 + 口径反转清算门` | — |
| 68 | `设计目标棘轮 + 实现名词体检` | — |
| 69 | `约定22 绕过反向判据 + 约定24 提交前门禁` | — |
| 70 | `文档编号连续性 + 检查脚本阳性对照骨架` | — |
| 71 | `yield 守卫棘轮 + HAS_WAKE_SOURCE 供给链` | — |
| 72 | `memory 整段被吞：确定性落点` | — |
| 73 | `约定 22 绕过检出：补「只改代码、册子零动」这一最常见形态` | — |
| 74 | `隐形漂移：内容变了但 git status 报 clean` | — |
| 75 | `失败处置五步：记账 → 判阈 → 冻结四件套 → 发 #4 → 让位` | — |
| 76 | `Mock 守卫策略按标记分流（两类目的相反，混用即出事）` | — |
| 77 | `收敛进脚本后，各门仍看得见` | — |
| 78 | `失败处置：--freeze-now / --extra / 枚举不可解析` | — |
| 79 | `CommonMark 围栏判定 + env/.env 键集新鲜度` | — |
| 80 | `收尾门 3p/3q 缺陷分流 + 上轮 block 复评` | — |
| 81 | `classify_push 同 build 多次推送累积` | — |
| 82 | `口径残留门 短形式词表 + 判决行` | — |
| 83 | `exec 报告继承 cases[]/testSummary` | — |
| 84 | `notify --section-file + 渲染冒烟检测面` | — |
| 85 | `设计目标三条形式落地的收口` | — |
| 86 | `--reset-* 旗标执行器` | — |
| 87 | `冻结字段写入契约` | — |
| 88 | `SKILL 阻断名单棘轮` | — |
| 89 | `术语一致性 + 约定 30 正文体检` | — |
| 90 | `台账多态识别 + 格式漂移告警` | — |
| 91 | `收尾门 3n 测试结果挂靠用例基线` | — |
| 92 | `子 Agent 派单契约（cascade_mode / dev_scale）` | — |
| 93 | `业务计数声明表全库回扫` | — |
| 94 | `约定 30 棘轮键：内容指纹而非行号` | — |
