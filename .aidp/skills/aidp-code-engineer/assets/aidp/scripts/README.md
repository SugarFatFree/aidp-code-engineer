...
输出字段：`commit_gate_enabled` / `is_template_project`（`{{AIDP_HOME}}/../skills/aidp-code-engineer/SKILL.md` 与 `scripts/sync_memory_md.py` 都在、且 `scaffold_marker.py` 判为非下游）/ `working_tree_dirty` / `has_business_code_change` / `is_scaffold_only_change` / `today` /
