...
用法:
  python scan_mock_data.py [代码目录] [--json] [--strict] [--third-party-mode] [--max-overdue-days N]
...
第三方临时 mock 模式 (--third-party-mode):
  对照 references/third-party-mock-protocol.md,识别带 THIRD_PARTY_MOCK 标注的代码:
...
参数:
  ...
  --max-overdue-days  第三方 mock expected_ready 过期天数阈值(默认 14;超过判 Critical 长期挂账)

跨平台支持: Linux / macOS / Windows (Python 3.8+)
"""
...
REAL_CLIENT_SIGNALS = [
    ...
    # 说明:海关单一窗口用 customs.gov.cn(.cn 结尾),故 TLD 放宽为 com|cn;
    #       原先把 customs.gov 混进 vendor 组再强制 \.com,只能匹配不存在的 customs.gov.com(死分支),已修正。
    (r"https?://(?!.*(mock|fake|example\.com|localhost|127\.0\.0\.1))[\w.-]*\.(alipay|weixin|qq|unionpay|tencentcloudapi|aliyuncs|aliapi|aliexpress)\.(?:com|cn)", "真实第三方 vendor 域名"),
    (r"https?://(?!.*(mock|fake|example\.com|localhost|127\.0\.0\.1))[\w.-]*customs\.gov\.cn", "真实海关单一窗口域名"),
]
    ...
    for i, line in enumerate(text.splitlines(), 1):
        for pattern, desc in patterns:
            if re.search(pattern, line):
                ...
                break  # 同一行只报一次,避免多个 pattern 命中导致重复计数
    return hits
...
def check_fake_api_functions(root: Path) -> list:
    ...
    # 只匹配"字面量假数据"(数组/对象字面量),不匹配 `return { data: res.data }` /
    # `return [...arr]` / `return [realVar]` 这类真实数据透传/包装,避免把正常 API 封装误判为 Critical。
    patterns = [
        (r'Promise\.resolve\s*\(\s*[\[\{]', "API 层 Promise.resolve 直接返回字面量假数据"),
        (r'return\s+\{[^}]*data\s*:\s*\[', "API 层返回硬编码 data 数组字面量 (return { data: [...] })"),
        (r'''return\s+\[\s*[\{'"0-9]''', "API 层返回硬编码数组字面量 (对象/字符串/数字元素)"),
    ]
    hits = []
...
# 对照 references/third-party-mock-protocol.md
# ---------------------------------------------------------------------------
...
# ── DEV_MOCK(同项目后端未部署)的**最小识别器**:只为产出 2A 豁免,不做任何协议判定 ──
#
# ⚠️ **为什么必须有这一段**:`DEV_MOCK` 是维度 2B 管理的合规形态
#    (dev-execution-planner/references/flow-edge-cases.md 情况 1 的两种 ✅ 推荐写法),
#    若它在 2A 这边没有豁免,那两种推荐写法喂进 `scan_mock_data.py --third-party-mode`
#    会被判 🔴 Critical + exit 1 ——「照着示例写反被扫描器判 Critical」,
#    正是 SKILL 在 `--third-party-mode` 那段写明要防的后果:
#      「漏加会把**完全合规的 2B mock** 判成 2A Critical + exit 1,验收循环把合规代码
#        打回"修复" —— 等于 2B 整档被 2A 顶掉」。
#
# ⚠️ **刻意不套 THIRD_PARTY_MOCK 那套语义**:`expected_ready` 过期、`coexist_with_real`
#    (真实 vendor SDK 并存)对 DEV_MOCK 都不适用 —— 它讲的是**同项目后端**,
#    与某个厂商 SDK 在不在同一个仓里毫无关系。字段完整性 / 计龄 / 反向判据一律由
#    维度 2B 的 `scan_third_party_mock_antipatterns.py` 承担,**本处只回答「该不该豁免 2A」**。
#
# ⚠️ **只有字段完整的块才买得到豁免** —— 缺字段的 DEV_MOCK 不进 exempt_files,
#    否则「写个裸标记就能关掉 2A」会变成一条后门;缺字段本身由 2B 判 Critical。
DEV_MOCK_REQUIRED_FIELDS = ['since', 'owner', 'REMOVE_WHEN']
DEV_MOCK_MARKER = re.compile(r'DEV_MOCK\s*[:：]')
# ⚠️ 与 `scan_third_party_mock_antipatterns.py` / `check_third_party_mock_runtime.py` 同款前缀护栏:
#    token 之前只能有空白与注释符号。`DEV_MOCK` 是个**极自然的常量名**
#    (`const DEV_MOCK: boolean = ...` / Go 的 `case DEV_MOCK:`),不加护栏会把普通业务代码
#    当成标注块。
DEV_MOCK_PREFIX_OK = re.compile(r'^[\s/*#\-<!;%]*$')
# ⚠️ **这是同款正则的第三份副本,但它刻意停在"纯前缀判定"、不配块注释状态机** ——
#    权威的那一对在 `scan_third_party_mock_antipatterns.py` 与
#    `dev-logic-architect/scripts/check_third_party_mock_runtime.py`(两处逐字节相同),
#    那两处必须配状态机,因为纯空白前缀会让 Python 类型标注 `<marker>: bool = True`
#    被判 `missing_fields`/Critical(假阳打在正常业务代码上)。
#    **本处不需要**:这个识别器**只产出 2A 豁免**,而豁免要求三字段齐全 ——
#    一行类型标注后面没有 since/owner/REMOVE_WHEN,拿不到豁免,既不假红也不假绿。
#    ⛔ 别"顺手统一"成带状态机的版本再漏改另两处,那才是漂移的开始;
#    要改就三处一起改,并重跑两侧各 5 条验收对照。
DEV_MOCK_FIELD_LINE = re.compile(
    r'^\s*[#/*\s]*(' + '|'.join(DEV_MOCK_REQUIRED_FIELDS) + r')\s*[:：]\s*(.+?)\s*$')


def find_dev_mock_files(root: Path) -> set:
    """返回含**字段完整** DEV_MOCK 标注块的文件路径集合(用于 2A 豁免)。"""
    hits = set()
    for fp in walk_files(root):
        try:
        ...
        for i, line in enumerate(lines):
            m = DEV_MOCK_MARKER.search(line)
            if not m or not DEV_MOCK_PREFIX_OK.match(line[:m.start()]):
                continue
            fields = set()
            for j in range(i + 1, min(i + 9, len(lines))):
                if DEV_MOCK_MARKER.search(lines[j]):
                    break
                fm = DEV_MOCK_FIELD_LINE.match(lines[j])
                if fm:
                    fields.add(fm.group(1))
            if all(f in fields for f in DEV_MOCK_REQUIRED_FIELDS):
                hits.add(str(fp))
                break
    return hits
...
def find_real_client_signals(root: Path):
    ...
    # 后端 application.yml / 前端 .env 常在项目根(src/ 的父级),扫 src/ 时回退到父级,
    # 与 scan_third_party_mock_antipatterns.py 的 cfg_root 语义对齐,避免漏掉根级真实凭证/base URL。
    config_names = re.compile(r'(application.*\.(yml|yaml|properties)|\.env(\..*)?|config.*\.(yml|yaml|properties))$',
                              re.IGNORECASE)
    config_root = root.parent if root.name == 'src' else root
    seen_cfg = set()
...
def run_scan(scan_dir: Path, output_json=False, strict=False, third_party_mode=False,
    ...
    exempt_files = set()
    if third_party_mode:
        third_party_result = check_third_party_mock(scan_dir, max_overdue_days)
        ...
        # 合规标注(字段完整、未过期、无真实 client 并存)的文件
        # 豁免普通 mock 零容忍检查(对应脚本 docstring 与 SKILL 维度 2B 约定)
        if not third_party_result["coexist_with_real"]:
            exempt_files = {p["file"] for p in third_party_result["compliant_pending"]}
        # ⚠️ DEV_MOCK(同项目后端未部署)同样豁免 2A —— 它是维度 2B 管理的合规形态。
        #    **无条件叠加、不受 `coexist_with_real` 影响**:那个条件讲的是「第三方真实 SDK
        #    已经接上了」,与同项目后端部署与否毫无关系,拿它去卡 DEV_MOCK 是张冠李戴。
        #    计数刻意**不加**进 total_issues —— DEV_MOCK 的字段/计龄/反向判据由 2B 的
        #    `scan_third_party_mock_antipatterns.py` 登记,2A 这边只负责「别把它判死」。
        exempt_files |= find_dev_mock_files(scan_dir)

    def _is_exempt(hit):
        f = hit["file"]
        if f in exempt_files:
            return True
        # 目录级命中(如 mock 目录):目录下全部代码文件均为合规第三方 mock 时豁免
        if hit.get("line") == 0 and os.path.isdir(f):
            inner = [str(p) for p in walk_files(Path(f))]
            return bool(inner) and all(p in exempt_files for p in inner)
        return False

    # 普通 mock 零容忍检查
    for name, check_fn, severity in CHECKS:
        hits = check_fn(scan_dir)
        if exempt_files:
            hits = [h for h in hits if not _is_exempt(h)]
        results.append({
            "check": name,
            "severity": severity,
            "issues": hits,
    ...
    if output_json:
        output = {
            ...
            # ⚠️ 「0 命中」须能与「0 文件被扫(路径给错)」区分:本脚本是维度 2A(Critical 零容忍)的
            #    采集侧,路径给错时早期同样输出 total_issues=0 + passed=true → 报告照抄「✅ 无残留」。
            "scanned_files": len(list(walk_files(scan_dir))),
            "skipped": len(list(walk_files(scan_dir))) == 0,
...
示例:
  ...
  python scan_mock_data.py src/ --third-party-mode --json
    ...
    run_scan(scan_dir, output_json=args.json, strict=args.strict,
