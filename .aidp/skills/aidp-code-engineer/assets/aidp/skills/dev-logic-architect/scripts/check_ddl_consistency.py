...
说明: DEFAULT 值与索引定义的逐项比对**暂未实现**(设计文档侧多不提供结构化 DEFAULT/索引描述,
      逐项比对易误报),本脚本仅核验字段清单/类型/NOT NULL —— compare() 从未读取 default/indexes。
      ⚠️ 此处曾写"DEFAULT 值是否一致 / 索引定义是否一致"，是**谎报能力**：QR Agent 据此认定
      已机检，实际零覆盖。与 dev-execution-planner 的姊妹副本保持同一结论。
...
def parse_design_doc(md_text: str, target_table: str | None) -> dict[str, dict]:
    ...
    # dev-logic-architect 的 A.3 canonical 输出形态就是 SQL 围栏里的 CREATE TABLE
    # (见 references/output-module-examples.md A.3 与 SKILL.md「生成 A.3 数据表时内联校验」),
    # 并非 markdown 字段表。若只认 markdown 表格,针对本 SKILL 自身模板产出的设计文档
    # 会恒判「未找到可对比的表」而失效。故此处对未从 markdown 表格取到的表,
    # 回退用同一套 DDL 解析器从设计文档的 SQL 围栏中提取。
