# .aidp/templates/ — 模板与骨架

本目录放**待填充的骨架**，不放规则本身。规则在 `.aidp/rules/`（路径限定）与
`.aidp/reference/`（查阅型分片）。
...
| `optional-rules/` | **按需安装规则的权威模板位** | 由各自的 `--install-rule` 装到 `.aidp/rules/` |
| `_开发期族增量.md` | 约定 22 四族增量册的空骨架 | 各族首次记增量时复制到对应族目录 |

## ⚠️ `optional-rules/` 是权威源，`.aidp/rules/` 下装的那份是副本

可选规则（如 WebMCP）默认**不在** `.aidp/rules/` —— 那里的加载是路径触发的，
只要编辑的文件命中 `paths:` 整份就进上下文，而绝大多数项目根本不启用这些能力，
读完的唯一结论是「本项目不适用」。故它们住在这里，启用后才安装过去。

**改动一律改这里**。装到 `.aidp/rules/` 的是副本：改了无法回流、下次刷新被覆盖。
