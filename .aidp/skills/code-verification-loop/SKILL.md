...
description: >
  ...
  发现问题后反馈修复团队迭代修复,循环直到全部通过(15 维度检查,含三个技术栈门控的部署/运行期盲区静态前置:Java/Spring 的 DI 依赖可解析性、Vue 的 CSS 预处理器一致性、前端的请求通道 URL 拼装单一信源;
  另含一个入参门控维度:客户端应用 MCP 业务能力实现合规,仅调用方显式 client_mcp.enabled: true 或旧 Web webmcp_enabled: true 时启用、否则整维度跳过不留痕;WebMCP 入口硬门只适用 Web;
  另含四个外部脚本门控维度:UI 还原度确定性检查(委派被测项目侧 check_ui_fidelity.py)、上游调用日志可见性与脱敏(委派被测项目侧 check_upstream_call_log.py,另加脱敏是否覆盖嵌套结构/二进制降级/拦截器覆盖面三条语义判据)、实现偏离设计(委派被测项目侧 check_design_anchor.py,**须额外传必填的 `--version <版本号>`**)、同族增量项一致性(委派被测项目侧 check_sibling_family.py,另加结构/文案骨架/交互流三条语义判据),四者未下发时均标 N/A 跳过但报告留行,
  ...
  当用户提到代码验收、功能检查、代码审查、验收检查、mock检查、第三方mock检查、接口对接验证、代码质量检查、字段对账、需求字段核对、DI 依赖检查、依赖注入可解析性、bean 找不到、启动期 bean 装配失败、CSS 预处理器检查、scss/less 预处理器未安装、vite build 失败、URL 拼接重复、baseURL 拼装、context-path 重复、请求 404 路径重复、UI 还原度、导出只导当前页、失效实体、已下架仍可点详情、加载态脏数据、编辑后页面不同步、存量数据兼容、同一指标跨页面不一致、上游调用日志、第三方调用没打日志、成功路径无日志、日志脱敏、凭据明文入日志、同族增量项、每版新增一个入口、兄弟组件不一致、新成员漏抄样式、成员顺序插错、公共外壳缺失时触发。
  当用户提到代码验收、功能检查、代码审查、验收检查、mock检查、第三方mock检查、接口对接验证、代码质量检查、字段对账、需求字段核对、DI 依赖检查、依赖注入可解析性、bean 找不到、启动期 bean 装配失败、CSS 预处理器检查、scss/less 预处理器未安装、vite build 失败、URL 拼接重复、baseURL 拼装、context-path 重复、请求 404 路径重复、UI 还原度、导出只导当前页、失效实体、已下架仍可点详情、加载态脏数据、编辑后页面不同步、存量数据兼容、同一指标跨页面不一致、上游调用日志、第三方调用没打日志、成功路径无日志、日志脱敏、凭据明文入日志时触发。
---

### 10b. 同族增量项一致性（**维度 15**·被测项目 约定 20；无 `check_sibling_family.py` 时机器半场标「不适用」跳过、**仍留行**，15.1~15.3 照跑）

> ⚠️ **编号是 `10b` 而不是往后顺延**——刻意不动 11~13 的既有编号（下游按编号引用本模板，顺延会让每一条引用整体位移，与 `4b` / `6b` 同款处置）。

| 判据 | 状态 | 说明 |
|--------|------|------|
| F0 声明漂移 / F1 成员自带外壳 / F2 成员未登记 | ✅ | `check_sibling_family.py --json`:家族 1 个(隐藏运维页历史数据处理入口),成员 6 个,F0/F1/F2 = 0 项 |
| F3 顺序未数据化 / F4 疑似家族无公共外壳（Important） | 🟡 | 1 项 F3:顺序仍由模板书写位置决定,已要求改为注册表 `order` 字段驱动(不阻断) |
| 15.1 结构骨架一致 | ✅ | 新成员 V0.15.0 与 V0.13.0 / V0.14.0 并排读,区块层级与插槽用法同形 |
| 15.2 文案骨架一致 | ❌ | 既有 5 个成员说明均为「作用 + 影响范围 + 耗时预期」三段,新成员只有一句话;**判的是三段要素齐不齐,不是措辞雷同** |
| 15.3 交互流一致 | ✅ | 二次确认弹窗 / loading / 成功失败反馈 / 禁用条件四项与既有成员一致 |
| 上游契约消费 | 🟡 | architect 未产出「同族增量项声明表」,已回退纯代码判断并报「设计缺声明表」,不静默放过 |

根据 维度 0（静态基线门）+ 15 维度检查结果，生成以下之一（**维度 0 未过时直接出「不通过」、不必填后 15 维度**）：
...
digraph verification_loop {
    ...
    check [label="Step 3: Agent2 执行 维度 0 + 15 维度检查\n(本 Skill 的核心)"];
