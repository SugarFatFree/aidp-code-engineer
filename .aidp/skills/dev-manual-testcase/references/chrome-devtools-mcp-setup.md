...
- **落地要求**:AI 工具在每个关键步骤调用截图能力(如 `take_screenshot`)并保存到测试产物目录;命名主干为 `{用例ID}-step{步骤号}.{ext}`。Web Chrome DevTools 优先 `TC-USER-001-step3.webp`(quality=90),不支持 WebP 的移动/小程序/Playwright 等驱动使用原生 `TC-MOBILE-002-step2.png`;已有 `.jpg/.jpeg` 可兼容但不作为 UI 默认格式。每个步骤只登记一个 artifact、不生成多格式副本;本 SKILL 不实现截图或转码,真实路径以 auto-test-runner 的 `evidence[].artifact` 为准
