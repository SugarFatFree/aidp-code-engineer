...
- Git 仓库（`git config user.name` 已设置）
- Python ≥ 3.9
...
脚手架还支持 `migrate`（已有项目改建）与 `upgrade`（旧版 AIDP 升级）。执行完成后会自动校验目录完整性。
...
| 7×24 | `python3 .aidp/scripts/aidp_scheduler.py install`（调度 `/sprint-autopilot` + `/sprint-aiauto-test`）| 全自动开发链路 / 全自动测试链路 |
...
```bash
python3 .aidp/scripts/aidp_scheduler.py install [--agent claude|codex|dsh]   # 两条链路各装一个用户级定时任务（默认 10m / 5m）
python3 .aidp/scripts/aidp_scheduler.py status                               # 定时任务 + 两条链路心跳
python3 .aidp/scripts/aidp_scheduler.py uninstall
```

- Linux 用 systemd --user timer（无 systemd 时 crontab）、macOS 用 launchd、Windows 输出 `schtasks` 命令；每个任务调用 `.aidp/scripts/agent_loop.sh --once <命令> --unattended`，自动补 `--no-loop`、flock 互斥、日志落 `memory/.aidp/logs/`。
...
里程碑通知由 `.aidp/scripts/notify.py` 发送，在 `memory/aidp-config.yaml` 的 `notify` 段配置，按顺序尝试、成功即停：
...
推送正式代码后，由 `.aidp/scripts/cicd_watch.py` 按 commit 锁定本次流水线运行、轮询至终态、失败自动重试（上限 `max_retries`），再跑部署就绪探针。平台差异收在 `.aidp/scripts/cicd_providers.py`，在 `memory/aidp-config.yaml` 的 `cicd` 段选择提供方：
...
```bash
python3 .aidp/scripts/cicd_watch.py --mode watch   --commit <sha> --env test    # 探测 + 轮询至终态
python3 .aidp/scripts/cicd_watch.py --mode trigger --env test --ref <branch>    # 主动触发
python3 .aidp/scripts/cicd_watch.py --mode retry   --env test --run-id <id>     # 重试失败的运行
python3 .aidp/scripts/cicd_watch.py --selftest                                  # 离线自测
```
...
`/sprint-aiauto-test` 依赖 chrome-devtools-mcp。它随仓库分发在 `.aidp/plugins/chrome-devtools-mcp/`，跑 `python3 .aidp/scripts/agent_sync.py`（脚手架 init / migrate / upgrade 会自动跑）即按当前 Agent 生成项目级入口；本机需要 Node.js（`npx`）。Codex 只在项目被标记为 trusted 后加载项目级 `.codex/` 配置。选择 DeepSeek Harness 执行 init 时，脚手架会尝试安装 `dsh-plugin-commands@latest`；安装失败会给出 WARN 与重试命令，不阻断项目文件生成。
