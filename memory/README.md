...
> ℹ️ 本仓库是模板项目，自身不跑版本规划与 Sprint，故只保留 `README.md` 与 `aidp-config.yaml`。下游项目由脚手架 `init` 铺设项目级记忆文件（`projectBrief.md` 等），迭代级文件（`{version}/{user}/activeContext.md`、`progress.md`）由 `/sprint-start`、`/memory-sync` **首次运行时自动创建**，`sprints/sprint-{NNN}.md` 由 `/sprint-close` 归档时写入。
<!-- TEMPLATE-ONLY:END -->
