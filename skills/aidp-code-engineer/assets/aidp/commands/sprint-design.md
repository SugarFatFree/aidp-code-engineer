...
- `--unattended`（可选）：★ **无人值守上下文标记**（由 `/version`·`/sprint-autopilot`·`/sprint-test` 等上游透传）——本命令内**一切 `AskUserQuestion` 与"暂停待用户裁决"一律不弹**，改按下方各门的确定性默认自动推进并留痕；**绝不静默跳过质量门**，只是把"问人"换成"按保守默认走 + 记录"。⛔ 上游传了而本命令不认，flag 会被静默丢弃、在无人值守链里挂死。
  ...
  - **★ 缓存 / 锁策略两道确认门**（`check_cache_user_confirmed` / `check_lock_strategy`）→ 无人值守**取保守默认**（不启用缓存 / 锁按 SKILL 核心原则 24 落 **P0 进程内锁**）+ **登记进 `99_待澄清问题清单.md`**，不阻塞、不静默采用激进值。⛔ 「保守」不等于「最严格」：P2 数据库级锁是核心原则 24 里**唯一必须人为确认**的一档，无人值守自动选它 = `check_lock_strategy.py` L1 每轮必判 Critical。
...
> 🛑 关键规则：跑完 Step 2~6 后**必须在此暂停** —— 先跑「A 回检清单」bash 块逐行填「🔍 输出前回检」表（12 项，单一信源 = `flows/sprint-design/step-输出前硬门.md` 的判定表），再跑「B 三合一回写校验」脚本（基线 + ADR + 金额字段），**B 块退出码必须为 `0` 且看到 `✅ 基线 + ADR + 金额字段 三合一回写校验通过` 字样**才算完成；任一 Fail → 输出"❌ 回检未通过：缺失项 …"停下请用户裁决，禁用"基本通过""主要项通过"等模糊措辞绕过。**跳过本硬门 = 严重违规**。

| 段 | 骨架 |
|----|------|
| A 回检清单 | 12 项判定表（文件数+NN_前缀 / 拆分数量 / L1 头部 / 对外接口 / 第三方边界 / 字典枚举 / ER·FK·NOT NULL / 文本长度 / **列表页数据量级与分页** / 待澄清清单 / L2·L3 溯源覆盖率 / 金额整数化）+ 逐项「🔍 输出前回检」输出格式。⛔ 项数与逐项判据以 flow 判定表为准，命令端只给索引；漏项的失效形态是**假绿**（如「列表页数据量级」的 S1 判 Important、不占退出码，不照表逐项填就读成通过）|
