...
- **用例模式必须展示进度 `<当前>/<总数>`**：总数 = 本角色加载的用例数 `TOTAL`（Phase 2.0 加载用例时得出）；每条用例**开始**打 `⏳ [角色] 进度 5/98 — TC-005 <标题> … 开始`、**结束**打 `✅/❌ [角色] 进度 5/98 — TC-005 <pass/fail>（耗时 Ns）`。多角色时每个角色各自从 1 计到该角色 `TOTAL`，角色切换时先打印该角色用例总数。
