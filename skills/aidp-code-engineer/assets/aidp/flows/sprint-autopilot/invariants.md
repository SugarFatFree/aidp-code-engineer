...
>   - **`trigger`（首次主动触发，构建分支当前 HEAD）**：执行输出里的 `trigger_cmd`，即 `cicd_watch.py --mode trigger --env <env> [--ref <branch>]`（流水线取 `memory/aidp-config.yaml` 的 `cicd.pipelines.<env>`）→ `next_action=poll` 使用 `--mode poll --run-id <输出的 run_id> --commit <本次 push_commit>`；`next_action=detect` 先跑 `--mode detect --commit <本次 push_commit> --env <env>` 锁定新 run id，再带同一 commit 续 poll。
