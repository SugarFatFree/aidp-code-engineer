...
|-------|------------|----------|---------------------|
| Claude Code | `.claude/skills/` | `.claude/commands/*.md` | `.claude/plugins/<name>/` |  # runtime-path-ignore: 适配位对照，必须逐字写出各 Agent 的目录
| Codex | `.agents/skills/` | `.codex/skills/aidp/<command>/SKILL.md` | `.agents/skills/<name>/skills/` + `.codex/config.toml` MCP |  # runtime-path-ignore: 适配位对照，必须逐字写出各 Agent 的目录
| DeepSeek Harness | `.agents/skills/` | `.dsh/commands/*.md` | `.agents/skills/<name>/skills/` + `.dsh/mcp.json` |  # runtime-path-ignore: 适配位对照，必须逐字写出各 Agent 的目录
...
# 运行根即 Agent 自己的目录（契约平铺其下）。字面量刻意拆写是为了躲开
# check_runtime_paths 的"硬编码旧运行根"正则 —— 这里是定义处，不是引用。
AGENT_RUNTIME_HOME = {"claude": "." + "claude", "codex": "." + "agents",
                      "dsh": "." + "agents"}
...
# 预检期（运行包尚未铺出、manifest 还不存在）由调用方显式告知目标运行根。
# ⛔ 不能靠探测：那时 `<root>/.claude/.aidp-runtime.json` 必然不在，探测必然判非恒等，
#    于是预检把运行包**将要**铺出的条目当成"已存在的用户内容"拒绝——init 在装配前就死。
EXPECTED_RUNTIME_REL = None


def _project_runtime_rels(root: Path) -> set:
    """本项目里**全部**运行包的根相对路径（`.claude` / `.agents`）。

    ⛔ 必须是集合而不是单个：Claude 与 Codex/DSH 并存时项目里有**两个**运行包，
    只取其一会把另一个的目录当成"真适配位"，于是把它自己铺出来的条目判成用户内容拒绝覆盖。
    ⛔ 探测按受管 manifest，不按目录在不在：`.claude/` 在任何用过 Claude Code 的项目里都有。
    """
    if EXPECTED_RUNTIME_REL:
        return set(EXPECTED_RUNTIME_REL) if isinstance(EXPECTED_RUNTIME_REL, (set, frozenset)) \
            else {EXPECTED_RUNTIME_REL}
    found = set()
    try:
        found.add(RUNTIME_ROOT.resolve().relative_to(Path(root).resolve()).as_posix())
    except (ValueError, OSError):
        pass
    for home in sorted(set(AGENT_RUNTIME_HOME.values())):
        if (Path(root) / home / ".aidp-runtime.json").is_file():
            found.add(home)
    return found


def _adapter_is_identity(plan, rel: str, sub: str) -> bool:
    """适配落点与运行包真源是同一个目录（运行根降层后的常态）。

    ⛔ 命中时**整体跳过**，包括 `_prune`：那一步会把"不在契约清单里"的条目当成多余
    生成物删掉 —— 降层后同一个目录里还住着项目自有的 skill / 命令，以及脚手架自身。

    ⛔ 判据必须是「本项目的运行包到底在哪」，不能写死 `.claude/skills` 恒为恒等：
    旧模板目录形态的项目里，`.claude/skills` 是**真适配位**，跳过它等于不装配。
    """
    root = plan.root if plan is not None else Path(".")
    return any(rel == f"{home}/{sub}" for home in _project_runtime_rels(root))

def sync_skill_dir(plan: Plan, rel: str, preserve: set = None) -> list:
    if _adapter_is_identity(plan, rel, "skills"):
        return []
...
def _covered_by_dir_rules(text_outside: str, entry: str) -> bool:
    """entry（仓库相对路径）是否已被块外的锚定目录规则（如 `/.claude/`）整体忽略。"""  # runtime-path-ignore: 指 Agent 自身目录这一概念，非运行契约路径，两包均保持原样
...
def sync_claude_plugins(plan: Plan, plugins: list) -> list:
    ...
    if plugins and _adapter_is_identity(plan, CLAUDE_PLUGINS, "plugins"):
        return [f"{CLAUDE_PLUGINS}/{plugin.name}" for plugin in plugins]
    for plugin in plugins:
        name = plugin.name
...
def _validate_targets(root: Path, agents: list, plugins: list, plugin_skills: dict):
    # ⛔ 预检必须与同步端同口径认「恒等落点」：运行根降层之后，`.claude/skills` 既是
    #    Claude 的发现位、又是运行包自己的 skills 目录 —— 那里的条目由 render_runtime 铺出，
    #    天然没有 agent_sync 的生成标记。不认恒等就会在预检期一律报"已存在用户内容"，
    #    init 在装配前直接失败。
    class _P:
        pass
    _plan = _P()
    _plan.root = root
    if "claude" in agents:
        if not _adapter_is_identity(_plan, CLAUDE_SKILLS, "skills"):
        ...
        if not _adapter_is_identity(_plan, CLAUDE_COMMANDS, "commands"):
            for command in _command_files(root):
                target = destination / command.name
                if (target.exists() or target.is_symlink()) and command.name not in previous:
        ...
        if not _adapter_is_identity(_plan, CLAUDE_PLUGINS, "plugins"):
            for plugin in plugins:
    ...
    if "codex" in agents or "dsh" in agents:
        if not _adapter_is_identity(_plan, SHARED_SKILLS, "skills"):
            for name in _base_skill_dirs(root):
