...
def is_template_project(root: str) -> bool:
    ...
    （供其本地重跑 upgrade），仅靠"引擎存在"会把下游误判为模板项目。故：
      ① 某个 `SCAFFOLD_SKILL_ROOTS` 落点下 `SKILL.md` 与 `scripts/sync_memory_md.py` 都存在；
      ② 且下游标记判定为「非下游」（`scaffold_marker.py`，回落 `aidp_config.is_downstream`）。
    """
    has_engine = any(os.path.isfile(os.path.join(root, base, "SKILL.md"))
                     and os.path.isfile(os.path.join(root, base, "scripts/sync_memory_md.py"))
                     for base in SCAFFOLD_SKILL_ROOTS)
...
def _downstream_marker(root: str) -> bool:
    ...
    """
    marker = next((m for m in (os.path.join(root, base, "scripts/scaffold_marker.py")
                                for base in SCAFFOLD_SKILL_ROOTS) if os.path.isfile(m)), "")
    if marker:
        try:
