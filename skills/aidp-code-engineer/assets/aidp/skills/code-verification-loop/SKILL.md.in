...
> 即便命中前端栈，只要 `client_mcp.enabled` 与旧 Web `webmcp_enabled` 都没传 true，它就整维度跳过
> （且**连行都不留**，不同于维度 6/7/8 会在报告里标「跳过」）；**⛔ 严禁本 SKILL 自行探测是否启用**。
> 门控表与 6 项判据全文见 `references/dimension-6-9-stack-gated.md` 维度 9，跨端总纲见
> `references/flow-client-mcp.md`。
...
- **外部脚本门控(维度 10 / 维度 12 / 维度 13 / 维度 15)** → 被测项目未随脚手架下发 `<被测项目根>/{{AIDP_HOME}}/scripts/check_ui_fidelity.py`(维度 10)/ `check_upstream_call_log.py`(维度 12)/ `check_design_anchor.py`(维度 13)/ `check_sibling_family.py`(维度 15) 时标「不适用(未下发)」跳过、**不硬失败,但报告里仍留行**(⚠️ 与维度 9 的「连行都不留」刻意不同——留行才能让阅读者知道「这一档没跑过」);**退出码 2 是入参/环境错、不是维度违规**。维度 12 另叠一层**技术栈门控**:脚本只扫 `.java`/`.kt`,非 JVM 项目 `outbound_files: 0` → 同样标「不适用」跳过、仍留行;维度 13 另叠一层**入参门控**:调用方未传「验收版本号」时同样标「不适用(未取到 `--version` 值)」跳过、仍留行,⛔ 不得自拟版本号。⚠️ **维度 15 的脚本缺失只关掉它的机器半场,15.1~15.3 的语义核对照跑**。⚠️ **这四个维度都以「被测项目根」为脚本查找基准,不是「代码仓库/目录」**——AIDP 项目里两者常不是同一个目录,传错即三档一起静默落 N/A(见「输入要求」表)
...
> **先过维度 0 静态基线门**（细则见 `references/dimension-0-2-baseline-mock.md`，不计入 15 维度计数）。
...
| 维度 | 名称 | 门控 | 细则分片 |
| :-: | :- | :- | :- |
| 0 | 静态基线门（前置，不计入 15 维度） | 恒启用；未过直接「不通过」 | `references/dimension-0-2-baseline-mock.md` |
...
| 9 | 客户端应用 MCP 业务能力实现合规（WebMCP = Web 端实现） | 入参 `client_mcp.enabled` 或旧 Web `webmcp_enabled` | `references/dimension-6-9-stack-gated.md`（跨端总纲 `references/flow-client-mcp.md`） |
...
| 15 | 同族增量项一致性（被测项目 约定 20） | 外部脚本 `check_sibling_family.py`（缺失只关机器半场，15.1~15.3 照跑） | `references/dimension-10-14-delegated-semantic.md` |
