...
**B.2.zb 同族增量项声明表(出现「每版新增一个 X」类家族时强制,对应核心原则 34 / 检查项 41):**

> ⚠️ **编号用 `zb` 而非顺延**:与 `za` 同一条纪律——追加式编号不会让既有引用位移。

> **★ 判据是「有没有兄弟」,不是「我写了几遍」。** 家族**已有同构成员 ≥2 即适用**,
> **哪怕本次只新增 1 个**。⚠️ 这类场景里作者**主观上正在复用**(他确实是照着兄弟改的),
> 所以按「重复 N 次」设阈值的规则**一条都不会触发**——这正是它能连续 3 个版本复发的原因。
...
| :- | :- | :- | :- | :- |
| 运维历史数据处理入口 | `src/views/ops/MaintenanceEntryCard.vue`(外壳唯一产出;差异量由 `title`/`desc`/`action` 三个 prop 参数化) | `src/views/ops/entries/*Entry.vue` | `src/views/ops/entries/registry.ts` | 按注册表 `order` 字段升序,⛔ 不按代码里的书写顺序 |
| 版本变更区块 | `src/components/ReleaseBlock.vue` | `src/content/release/*.md` | `src/content/release/index.json` | 按 `releasedAt` 降序 |
```

**两条硬性约束(各对应一个连续复发的真实缺陷):**

1. **第 2 列「公共单元位置」** —— 外观/结构由**哪个唯一单元**产出,**并说明差异量如何参数化**。
   真实缺陷:家族没抽公共单元,父页样式是 `scoped`、**传不进子组件**,
   于是每个成员各自复制一份样式,**漏抄即失效**。
   「各成员自行实现」「见上」「框架自动处理」一律不算(硬门 S5)。
2. ⚠️⚠️ **第 5 列「排序规则」是最容易敷衍、也最该卡死的一列** —— 顺序由**什么数据**计算
   (注册表文件 / 排序键字段 / 排序函数)。⛔ **不接受「按代码里的书写顺序」
   「新增时插到对应位置」**——那等于**把顺序交给物理位置**,而**物理位置不是数据、
   没有任何机器能校验它**(硬门 S4)。真实缺陷:顺序靠模板里手工插位 + 一行注释提醒。

> **第 3~4 列是给下游机器门用的**:被测项目侧 `check_sibling_family.py` 按**成员 glob** 找成员、
> 按**注册表/排序数据源**判「成员未登记」与「顺序未数据化」。
> **设计期填了,开发期就能直接落成 `SIBLING-FAMILY:` 声明**;设计期不填,那道门无从工作。

> ⚠️ **失败形态是「看起来正常」**:类型检查 / lint / 单测全绿、构建通过,
> **只有人打开页面并排比对才看得出来**——这就是它值得在设计期花两行声明去堵的全部理由。

> **确无同族增量项时**须就地写明「本次无同族增量项」,不得整表省略。

> ⚠️ 上面的示例写在 ```markdown 围栏内(与 B.2.za 一致),而硬门会跳过代码围栏,故**它不是
> `check_sibling_family_spec.py` 的回归样本**——真正被硬门扫到的 canonical 样本是
> `quality-review-checklist.md` 检查项 41 正文里那张(围栏外,实测 rc=0)。
> 改本示例的列名时**那一张要同步改**,否则两张 canonical 会分叉。
> ⚠️ 硬门 `check_sibling_family_spec.py` **刻意不做缺表判定**,故 **`skipped=true` 不等于通过**,
> 表该不该有由 QR 子 Agent 按检查项 41 核验步骤 1 的人工枚举判。
> 硬门:`python3 <SKILL_DIR>/scripts/check_sibling_family_spec.py <设计文档路径或目录> --json`。
> **跨产物契约**:下游 `code-verification-loop` **维度 15** 以本表为基准
> (第 2 列是 F1/F4 的比对对象、第 4~5 列是 F2/F3 的比对对象)。
