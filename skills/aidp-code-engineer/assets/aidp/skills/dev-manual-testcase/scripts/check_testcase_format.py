...
KNOWN_CASE_TYPE_TAGS = (
    ...
    "[外部调用方]", "[应用MCP]",
    # [应用MCP] 只标被测应用自身暴露的工具;Chrome/Appium 测试驱动即使用 MCP 协议也不算。
    # 不新增动作动词豁免:外部调用方仍用已有「执行」,代码级动作照常拦。
    "[外部调用方]",
...
def extract_step_table(body: str):
    ...
    for end_marker in (
        ...
        r"\*\*日志可追溯\*\*\s*[::]",
        # 应用工具断言专段([应用MCP] 用例 / flow-client-mcp.md):段内要给出调用证据锚点,
        # 自然会引用上游响应的「method + /path」(如 `内含上游响应 `GET /api/orders` 的业务码`)。
        # 与上面两条是同一个失效形态——不剥离,照文档写的合规用例会被判 Critical(实测)。
        r"\*\*应用工具断言\*\*\s*[::]",
        r"\*\*回归路径\*\*\s*[::]",
        r"\*\*实际结果",
        r"^##\s",  # 二级章节
