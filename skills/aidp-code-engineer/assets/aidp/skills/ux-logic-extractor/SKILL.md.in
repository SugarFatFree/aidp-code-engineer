...
2. **增量产物命名续编最大序号** — 增量 PRD 产物与全量产物**同目录**,统一 `NN_<业务主题>.md` 纯数字前缀(两位序号在文件名最前),序号**接已有内容文档最大序号自动累进**(不跳号、不嵌套二级序号)。**文件名一律不带「补充」/「追加」字样**(遵循 [`references/flow-output-format.md`](./references/flow-output-format.md) 的「拆分原则」命名规则);增量身份靠「产物头部回链 + `00_索引.md` 追加行(类型=补充 + 生成时间)」体现,而非文件名前缀。**只在 `00_索引.md` 追加一行,不改内容主文档 `01_PRD-{项目名}.md` 正文。**
